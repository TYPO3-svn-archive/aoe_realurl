...
class tx_aoerealurlpath_hooks_crawler {
	...
	function insertPageIncache(&$pObj, $timeOutTime) {
		...
		if (
			...
			&& (
				...
				|| 
				in_array ( 'tx_aoerealurlpath_rebuild', $pObj->applicationData ['tx_crawler'] ['parameters'] ['procInstructions'] )
			)
		) {
	...
	public function headerNoCache($params, $tsfe) {
		 ...
		 if (
			...
			&& in_array('tx_aoerealurlpath_rebuild', $params['pObj']->applicationData['tx_crawler']['parameters']['procInstructions'])
		) {
